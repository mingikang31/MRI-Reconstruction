...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, channels, num_pool_layers, dropout, device):
        ...
        for _ in range(num_pool_layers - 1):
            self.encoder_layers.append(EncoderBlock(ch, ch * 2, dropout))
            ch *= 2
        self.encoder_layers.append(EncoderBlock(ch, ch * 2, dropout))  
        
        self.bottleneck = BottleneckBlock(ch * 2, ch * 2, dropout)  
        
        self.decoder_layers = nn.ModuleList()
        for _ in range(num_pool_layers - 1):
            self.decoder_layers.append(DecoderBlock(ch * 2, ch, dropout))
            ch //= 2
        self.decoder_layers.append(DecoderBlock(ch * 2, channels, dropout))  
...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout, use_residual=True):
...
if __name__ == "__main__":
    ...
    model = UNet(in_channels=1, out_channels=2, channels=8, num_pool_layers=5, dropout=0.2, device=device) 
